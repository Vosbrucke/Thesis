...
df <- read_csv("Processed_data/df_log.csv")
...
sf::st_write(df_shp, "Processed_data/df_shp_log.shp") #, driver = "ESRI Shapefile", append = FALSE)

# Write column names (shapefiles are limited to 10 characters- column names in st_write funtion get abbreviated)
write_csv(as.data.frame(colnames(df_shp)), "Processed_data/df_log_shp_colnames.csv")
