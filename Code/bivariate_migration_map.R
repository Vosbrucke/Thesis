...
## Spain
data_y <- df_spain %>% 
  filter(type == "EP", year > 2014) %>% 
  mutate(values_y = 100 * foreign_born_population) %>% 
  select(geo = nuts2016, values_y, year)

data_x <- df_spain %>% 
  filter(type == "EP", year > 2014) %>% 
  select(geo = nuts2016, values_x = growth_farright_p_perc, year)
...
subtitle <- "Spain, Europarliament elections"
...
ggsave("Plots/Bivariate_maps/Spain_ep.png", dpi = 900, height = 22, width = 22, units = "cm")

# Parliamentary elections
data_y <- df_spain %>% 
  filter(type == "Parliament", year > 2016) %>% 
  mutate(values_y = 100 * foreign_born_population) %>% 
  select(geo = nuts2016, values_y, year)
...
legend_x <- "lower → higher\ngrowth of far-right parties"
title <- "Foreign-born population and growth of far-right political movement (%)"
subtitle <- "Spain, Parliamentary elections"
caption <- "Source: Eurostat, national statistic office"

bivariate_map_eu(data_x = data_x, data_y = data_y, legend_x = legend_x, legend_y = legend_y, title = title, subtitle = subtitle, caption = caption)

ggsave("Plots/Bivariate_maps/Spain_parliament.png", dpi = 900, height = 22, width = 22, units = "cm")


## Germany
data_y <- df_germany %>% 
  filter(type == "EP") %>% 
  mutate(values_y = 100 * (third_countries_to_eu_28_until_january_31_2020)) %>% 
  select(geo = nuts2016, values_y, year)
...
legend_x <- "lower → higher\ngrowth of far-right parties"
title <- "Foreign-born population and growth of far-right political movement (%)"
subtitle <- "Germany, Europarliament elections"
caption <- "Source: Eurostat, national statistic office"

bivariate_map_eu(data_x = data_x, data_y = data_y, legend_x = legend_x, legend_y = legend_y, title = title, subtitle = subtitle, caption = caption)

ggsave("Plots/Bivariate_maps/Germany_ep.png", dpi = 900, height = 22, width = 22, units = "cm")
...
legend_x <- "lower → higher\ngrowth of far-right parties"
title <- "Foreign-born population and growth of far-right political movement (%)"
subtitle <- "Germany, Parliamentary elections"
caption <- "Source: Eurostat, national statistic office"

bivariate_map_eu(data_x = data_x, data_y = data_y, legend_x = legend_x, legend_y = legend_y, title = title, subtitle = subtitle, caption = caption)

ggsave("Plots/Bivariate_maps/Germany_parliament.png", dpi = 900, height = 22, width = 22, units = "cm")


## Hungary
data_y <- df_hungary %>% 
  filter(type == "EP", year > 2002) %>% 
  mutate(values_y = 100 * (perc_foreign_born_population)) %>% 
  select(geo = nuts2016, values_y, year)
...
legend_x <- "lower → higher\ngrowth of far-right parties"
title <- "Foreign-born population and growth of far-right political movement (%)"
subtitle <- "Hungary, Europarliament elections"
caption <- "Source: Eurostat, national statistic office"

bivariate_map_eu(data_x = data_x, data_y = data_y, legend_x = legend_x, legend_y = legend_y, title = title, subtitle = subtitle, caption = caption)

ggsave("Plots/Bivariate_maps/Hungary_ep.png", dpi = 900, height = 22, width = 22, units = "cm")

# Parliamentary elections
data_y <- df_hungary %>% 
  filter(type == "Parliament", year > 2002) %>% 
  mutate(values_y = 100 * (perc_foreign_born_population)) %>% 
  select(geo = nuts2016, values_y, year)
...
legend_x <- "lower → higher\ngrowth of far-right parties"
title <- "Foreign-born population and growth of far-right political movement (%)"
subtitle <- "Hungary, Parliamentary elections"
caption <- "Source: Eurostat, national statistic office"

bivariate_map_eu(data_x = data_x, data_y = data_y, legend_x = legend_x, legend_y = legend_y, title = title, subtitle = subtitle, caption = caption)

ggsave("Plots/Bivariate_maps/Hungary_parliament.png", dpi = 900, height = 22, width = 22, units = "cm")
...
data_y <- df_italy %>% 
  filter(type == "EP") %>% 
  mutate(values_y = 100 * (perc_foreign_born_population), year = 2019) %>% 
  select(geo = nuts2016, values_y, year)
...
legend_x <- "lower → higher\ngrowth of far-right parties"
title <- "Foreign-born population and growth of far-right political movement (%)"
subtitle <- "Italy, Europarliament elections"
caption <- "Source: Eurostat, national statistic office"

bivariate_map_eu(data_x = data_x, data_y = data_y, legend_x = legend_x, legend_y = legend_y, title = title, subtitle = subtitle, caption = caption)

ggsave("Plots/Bivariate_maps/Italy_ep.png", dpi = 900, height = 22, width = 22, units = "cm")

# There's no parliamentary elections data for Italy
